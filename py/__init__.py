...
from core import Handler, Request, Response, BytesIOToBytes

default_font, font_size = Path(os.path.dirname(os.path.abspath(__file__))) / "font" / "consola.ttf", 15
...
async def create(request: Request) -> Response:
    if request.event.quote.imageList:
        pic = Image.open(io.BytesIO(request.event.quote.imageList[0]))
    else:
        pic = Image.open(io.BytesIO(request.event.imageList[0]))
    ...
    return Response(image=BytesIOToBytes(res))
